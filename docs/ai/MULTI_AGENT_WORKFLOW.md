...
适用于本仓库的后续开发。配置已写入仓库；M0–M9 的运行时实现和设备验收仍未开始。首次实施从[首批工作卡](FIRST_BATCH.md)开始，完整工程要求见[实施指南](IMPLEMENTATION_GUIDE.md)。

## 1. 默认分工

推荐 **1 位 Sol 协调者 + 2 位 Sol 开发者 + 按需调用的 Astra**。工作不能独立拆分时由一个 Sol 完成。Astra 不需要持续读取所有开发日志，也不需要在每次小修复后重审整个项目。

| 角色 | 默认模型 / 强度 | 职责 | 调用时机 |
| --- | --- | --- | --- |
| 主协调者 / integrator | GPT-5.6 Sol / medium | 拆工作卡、分配文件、汇总证据、集成与更新状态 | 每个批次 |
| `fsr4_worker`，通常两位 | GPT-5.6 Sol / medium | 独立实现和必要测试，短交接 | 有独立工作包时并行 |
...
仓库的 [.codex/config.toml](../../.codex/config.toml) 将主模型和默认子模型设为 Sol medium，子智能体并发上限设为 3（不含主协调者）。实际可用槽位取配置、客户端和宿主限制中更严格者。通常先运行两个 worker，完成后再调用一个 reviewer，无需填满槽位。
...
3. 若当前宿主没有暴露自定义角色选择，使用可用的原生子智能体工具，显式指定 `gpt-5.6-sol` 和 `medium`；审阅显式指定 `gpt-6-astra` 和 `high`。若工具支持控制历史，采用新上下文/最少历史并传入完整工作卡；不要复制整段聊天。
4. 创建后核对角色/模型信息或启动参数，记录实际执行模型。配置不支持或模型不可用时报告具体原因，不能静默改为 Astra Ultra 再声称使用 Sol。
5. 若宿主无法在子智能体间混用模型，可手动开两个 Sol 任务与一个 Astra 审阅任务，使用独立 worktree 和同样的文件交接。自动新建用户可见任务仍应遵从用户的明确请求，不由工作流擅自扩张。
...
1. **协调者建卡。** 按[工作包模板](../templates/work-package.md)写清目标、基准提交、依赖、读取章节、允许修改的路径、验收、退出条件与报告路径。先冻结两位 worker 共同依赖的最小接口，再并行。
...
```text
按 docs/ai/MULTI_AGENT_WORKFLOW.md 推进 docs/ai/FIRST_BATCH.md 的首批 M0 工作。
你是 Sol 协调者。先读 AGENTS.md、STATUS 和首批工作卡，检查当前文件与环境。
先完成共享接口的小范围冻结，再给两位 fsr4_worker 分配互不重叠的路径。
每个 worker 用 GPT-5.6 Sol / medium，只传所需工作卡和读取章节，不传完整聊天。
共享工作区由你独占 Git 索引、公共合同与状态页；worker 只写自己的源码/测试/报告。
本批不要求设备已连接，主机检查与 Odin 3 device_test 必须分开记录。
集成并完成主机检查后，冻结 diff，生成 review-packet，调用 fsr4_reviewer
（GPT-6 Astra / high）审实际源码与证据；你负责修复和最终集成。
遇到架构争议时给 fsr4_architect 最小证据包，不反复全仓重审。
交付变更、实际验证、未测项、短交接和下一步，不提前标 M0 真机完成。
```
...
本轮使用显式模型参数成功派发两位 Sol 编写模板与首批计划，证明当前会话可请求混合模型协作；仓库 TOML 还需要在下一次加载该项目的会话核对是否被宿主采用。语法/本地配置检查不等于已经完成一次使用这些自定义角色的运行时开发验收。

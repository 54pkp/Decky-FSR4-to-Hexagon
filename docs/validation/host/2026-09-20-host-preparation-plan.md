...
- 基准 `cc51b52`；仅修改本记录、STATUS 与 HOST_PREPARATION，不启动 P 批次。目标明确为 W8A8，核对固定上游的 8 位权重/激活参数与 INT8 Q/DQ，避免把两种描述误判为互斥格式；实际转换需求等待资产检查。
- 按个人本地实验简化获取：允许公开 fork/镜像，保留内容核验及随包通知，不另建许可审计。没有下载模型/SDK、运行转换或新增设备证据。
- 检查：`git diff --check` / `git diff --cached --check` 退出 0，14 个本地链接存在；纯文档修改未重跑 CPU suite。Astra medium 审阅三文件澄清 diff（tree `ff51c117604846cad4e86de5f7b9454bd8fb1a1e`）通过，无实质问题；其后仅补本条检查记录。

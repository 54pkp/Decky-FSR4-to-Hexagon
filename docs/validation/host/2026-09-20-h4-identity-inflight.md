...
- 实际模型/强度：协调 unknown / unknown；实现与只读审计 GPT-5.6 Sol / medium；批次审阅待冻结 diff 后由 GPT-6 Astra / medium 执行。
...
| `py -3 -m py_compile tools/replay/lifecycle.py tests/replay/test_lifecycle.py` | 0 | host_test / pass |

...
下一批：**R12a——将P9b消费/reset并发、25+窗口与namespace独立检查固化。**

| 队列 | 当前实现状态 | 选择条件 |
| --- | --- | --- |
| R：审计修复、可复现性与回归 | R01–R11 `complete`；其余叶子项 `not_started` | 当前优先；R12a起，独立项可按依赖选择 |
...
- P7 graph marker/count及NPZ 100-array/shape/dtype独立语义负例已由R09固化；P7环境/argv/stdout收据及P8对其固定摘要、来源和输出的信任链已由R10a/R10b固化。P9a历史附加资源检查已由R11固化；P9b/P9c并发与压力组合仍待R12a/R12b。多venv聚合检查已由R08a固化。
...
原始2026-09-22审计为基线233项、199 pass / 34 skipped；后续R批次持续增加回归。R11后当前本机基线为313项、255 pass / 58 skipped；需NumPy的R09/R10a/R10b方法在公开基线准确skip，在fsr-extract环境实跑。R08a本机四venv聚合为399次执行观察（351 pass / 48 skipped），其中存在跨组重叠。专用环境结果须单列，不能把skip写成pass。
...
最近记录：[R11 P9a资源预算回归固化](validation/host/2026-09-22-r11-p9a-budget-regressions.md)；前一批：[R10b P8 accepted receipt信任链](validation/host/2026-09-22-r10b-p8-accepted-receipt.md)。

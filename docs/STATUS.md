...
下一批：**R13——建立当前artifact索引并固定消费者选择。**

| 队列 | 当前实现状态 | 选择条件 |
| --- | --- | --- |
| R：审计修复、可复现性与回归 | R01–R12b `complete`；R13 `not_started` | 当前优先；完成R13后本队列结束 |
...
原始2026-09-22审计为基线233项、199 pass / 34 skipped；后续R批次持续增加回归。R12b后当前本机基线为321项、263 pass / 58 skipped；需NumPy的R09/R10a/R10b方法在公开基线准确skip，在fsr-extract环境实跑。R08a本机四venv聚合为399次执行观察（351 pass / 48 skipped），其中存在跨组重叠。专用环境结果须单列，不能把skip写成pass。
...
最近记录：[R12b P9c失败/关闭回归固化](validation/host/2026-09-22-r12b-p9c-close-regressions.md)；前一批：[R12a P9b有界幂等回归固化](validation/host/2026-09-22-r12a-p9b-idempotency-regressions.md)。

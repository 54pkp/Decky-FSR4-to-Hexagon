...
下一批：**R08b——固化P3后代进程继承管道超时回归。**

| 队列 | 当前实现状态 | 选择条件 |
| --- | --- | --- |
| R：审计修复、可复现性与回归 | R01–R08a `complete`；其余叶子项 `not_started` | 当前优先；R08b起，独立项可按依赖选择 |
...
原始2026-09-22审计为基线233项、199 pass / 34 skipped；后续R批次持续增加回归。R08a后当前基线为284项、236 pass / 48 skipped；本机四venv聚合为399次执行观察（351 pass / 48 skipped），其中存在跨组重叠。原Windows普通文件symlink固定skip在本机能力探测成功并实跑通过。专用环境结果仍须单列，不能把skip写成pass。
...
最近记录：[R08a 四venv聚合验证入口](validation/host/2026-09-22-r08a-multi-venv-verification.md)；前一批：[R07 Windows文件symlink能力探测](validation/host/2026-09-22-r07-windows-symlink-capability.md)。

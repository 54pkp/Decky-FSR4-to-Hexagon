...
下一批：**R12b——将P9c retained、四代预算及close/completion并发检查固化。**

| 队列 | 当前实现状态 | 选择条件 |
| --- | --- | --- |
| R：审计修复、可复现性与回归 | R01–R12a `complete`；其余叶子项 `not_started` | 当前优先；R12b起，独立项可按依赖选择 |
...
- P7 graph marker/count及NPZ 100-array/shape/dtype独立语义负例已由R09固化；P7环境/argv/stdout收据及P8对其固定摘要、来源和输出的信任链已由R10a/R10b固化。P9a/P9b历史附加资源与幂等检查已由R11/R12a固化；P9c close/failure并发压力仍待R12b。多venv聚合检查已由R08a固化。
...
原始2026-09-22审计为基线233项、199 pass / 34 skipped；后续R批次持续增加回归。R12a后当前本机基线为318项、260 pass / 58 skipped；需NumPy的R09/R10a/R10b方法在公开基线准确skip，在fsr-extract环境实跑。R08a本机四venv聚合为399次执行观察（351 pass / 48 skipped），其中存在跨组重叠。专用环境结果须单列，不能把skip写成pass。
...
最近记录：[R12a P9b有界幂等回归固化](validation/host/2026-09-22-r12a-p9b-idempotency-regressions.md)；前一批：[R11 P9a资源预算回归固化](validation/host/2026-09-22-r11-p9a-budget-regressions.md)。
